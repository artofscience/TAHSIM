...
from models import Source, Actuator, Actuation, Circulation, Circulations, Heart, Valve, BiVenSystemSimple, find_one_to_zero, find_zero_to_one, ejected_volume
from models import data0
...
tricuspid_valve = Valve((data["resistance tricuspid valve"] + data["systemic vascular resistance"]) / r_ref)

pulmonary_valve = Valve(data["resistance pulmonary valve"] / r_ref)

mitral_valve = Valve((data["resistance mitral valve"] + data["pulmonary vascular resistance"]) / r_ref)

# systemic and pulmonary circulation
systemic_circulation = Circulation(aortic_valve, tricuspid_valve, (data["systemic arterial capacitance"] + data["systemic venous capacitance"]) / c_ref,
...
pulmonary_circulation = Circulation(pulmonary_valve, mitral_valve, (data["pulmonary arterial capacitance"] + data["pulmonary venous capacitance"]) / c_ref,
...
# system
system = BiVenSystemSimple(actuation, heart, circulations)
...
vs0 = (data["systemic arterial unstressed volume"] + data["systemic venous unstressed volume"]) / v_ref

vp0 = (data["pulmonary arterial unstressed volume"] + data["pulmonary venous unstressed volume"]) / v_ref

total_volume = data["total blood volume"] / v_ref
unstressed_volume = vl0 + vr0 + vp0 + vs0
stressed_volume = total_volume - unstressed_volume

vp10 = vp0 + 2/7 * stressed_volume
vs10 = vs0 + 5/7 * stressed_volume

initial_guess_volumes = (0.0, 0.0, vl0, vr0, vs10, vp10)

no_actuation_cycles = 10
...
plt.plot(times, vvL + vvR + vp1 + vs1)
