...
from models import Source, Actuator, Actuation, Circulation, Circulations, Heart, Valve, BiVenSeptum, find_one_to_zero, find_zero_to_one, ejected_volume

from matplotlib import pyplot as plt

class Septum:
    def __init__(self, E, V0):
        self.E = E
        self.V0 = V0

class SeptumHeart(Heart):
    def __init__(self, left, right, septum):
...
data = {"relative duration systole": 1/3, # relative duration of systole vs full cycle
        ...
        "source pressure left ventricle": 250, # pressure of left source in mmHg (50 kPa)
        ...
        "pulmonary arterial unstressed volume": 100,  # unstressed volume of pulmonary arterial system in mL
        "pulmonary venous unstressed volume": 500,  # unstressed volume of pulmonary arterial system in mL
        ...
        "left ventricle elastance": 1, # elastance of left ventricle (if linear) in mmHg / mL
        "right ventricle elastance": 0.5,  # elastance of left ventricle (if linear) in mmHg / mL
        "septum elastance": 5.0 # somewhere in between left and right elastance
        }
...
svr = lambda t: (2.0 / r_ref + Sigmoid(-1.5 / r_ref, 20)(t) + Sigmoid(1.5/r_ref, 25)(t) +
                 Sigmoid(1.0 / r_ref, 40)(t) + Sigmoid(-1.0 / r_ref, 41)(t))
...
septum = Septum(data["septum elastance"] / e_ref, 0.0)

heart = SeptumHeart(left_ventricle, right_ventricle, septum)

# system
system = BiVenSeptum(actuation, heart, circulations)
...
plt.plot(times[index:], vvL[index:], 'r-', label='Volume ventricle left')
plt.plot(times[index:], vvR[index:], 'b-', label='Volume ventricle right')
vs = system.septum_volume(vvL, vvR, haL, haR)
plt.plot(times[index:], vs[index:], 'k-', label='Volume septum')
...
plt.figure()
plt.plot(times[index:], (vs1[index:] + data["systemic arterial unstressed volume"] + vs2[index:] + data["systemic venous unstressed volume"])/(vp1[index:] + data["pulmonary arterial unstressed volume"] + vp2[index:] + data["pulmonary venous unstressed volume"]))
...
plt.plot(times, svr(times), 'k:', label="SVR")
...
plt.plot(aligned_left / aligned_right,np.gradient(aligned_left/aligned_right))
