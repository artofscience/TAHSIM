...
from models import Source, Actuator, Actuation, Circulation, Circulations, Heart, Valve, BiVenSystem, find_one_to_zero, find_zero_to_one, ejected_volume
...
svr = lambda t: 2.0 / r_ref + Sigmoid(1.0 / r_ref, 20)(t)
...
plt.plot(times[index:], data["stroke volume"] * vvL[index:], 'r-', label="left ventricular volume")
# plt.axhline(heart.right.Vv0, linestyle='--', color='blue', label="RVv0")
plt.plot(times[index:], data["stroke volume"] *vvR[index:], 'b-', label="right ventricular volume")
plt.plot(times[index:], data["stroke volume"] *vp1[index:], 'b--', label="volume pulmonary arteries")
plt.plot(times[index:], data["stroke volume"] *vs1[index:], 'r--', label="volume systemic arteries")
plt.plot(times[index:], data["stroke volume"] *vaL[index:], 'r-.', label="left source volume")
plt.plot(times[index:], data["stroke volume"] *vaR[index:], 'b-.', label="right source volume")
...
plt.plot(data["stroke volume"] *(heart.left.Vp0 + heart.left.Vv0 - vvL[index:]), data["mean arterial pressure"] *haL[index:], 'r--', label="left pouch PV", alpha=0.1)
