...
from models import Source, Actuator, Actuation, Circulation, Circulations, Heart, Valve, BiVenPeri, find_one_to_zero, find_zero_to_one, ejected_volume
...
class PericardiumHeart(Heart):
    def __init__(self, left, right, pericardium):
...
data = {"relative duration systole": 1/3, # relative duration of systole vs full cycle
        ...
        "pericardium elastance": 0.1 # somewhere in between left and right elastance
        }
...
total_unstressed_ventricular_volume = data["ventricle left unstressed volume"] / v_ref + data["ventricle right unstressed volume"] / v_ref
pericardium = Pericardium(data["pericardium elastance"] / e_ref, total_unstressed_ventricular_volume)

heart = PericardiumHeart(left_ventricle, right_ventricle, pericardium)

# system
system = BiVenPeri(actuation, heart, circulations)
