...
fig, ax = plt.subplots()
cx = np.linspace(0, 3, 100)
vy = np.linspace(0,3, 100)
CX, VY = np.meshgrid(cx, vy)
plt.contourf(CX, VY, CX * VY)
plt.colorbar()
...
fig, ax = plt.subplots()
wx = np.linspace(0, 1.5 * pump.w0, 100)
ty = np.linspace(0,10, 100)
WX, TY = np.meshgrid(wx, ty)
plt.contourf(WX, TY, WX * TY /1e3)
plt.colorbar()
